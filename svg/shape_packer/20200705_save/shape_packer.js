...
// Draw the display
//-------------------------------------------------------------------------------------------------
ShapePacker.prototype.Draw = function (element)
{
    element.innerHTML = this.ToSVG (this.show_touching);
}
//-------------------------------------------------------------------------------------------------
// Draw the display
//-------------------------------------------------------------------------------------------------
...
// Convert to SVG
//-------------------------------------------------------------------------------------------------
ShapePacker.prototype.ToSVG = function (show_touching)
{
    var svg = SVGHelp.Start (500, 500, -1.02,-1.02, 1.02, 1.02);
    svg += this.outer_shape.ToSVG ();

    for (var i in this.shapes)
    {
        svg += this.shapes [i].ToSVG ();
    }
    ...
    if (show_touching)
    {
        for (var i = 0 ; i < this.shapes.length - 1 ; ++i)
        {
            ...
            for (var j = i+1 ; j < this.shapes.length ; ++j)
            {
                ...
                {
                    svg += SVGHelp.Line ([s1.x, s1.y], [s2.x, s2.y], "black");
                }
            }
        }
    }
