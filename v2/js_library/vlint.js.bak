...
VLInt.prototype.Divide = function (other)
{
    ...
    var len1 = other.value.length;
    var len2 = this.value.length;
    var two = new VLInt (2);
    var temp = new VLInt (this.value);
    var powers = [new VLInt (1)];
    var subs = [new VLInt (other.value)];
    var answer = new VLInt (0);
    var pos = 0;
    ...
    {
        subs.push (subs [pos].Multiply (two));
        powers.push (powers [pos].Multiply (two));
        ++ pos;
    }

    for (var i = pos ; i >= 0 ; --i)
    {
        if (temp.IsGreaterThanOrEqualTo (subs[i]))
        {
            answer = answer.Add (powers[i]);
            temp = temp.Subtract (subs [i]);
        }
    }
    return answer;
}
