...
ShapesController.t_phases = [0.01, 0.005, 0.002, 0.001, 0.0005, 0.0002, 0.0001, 0.00005, 0.00002, 0.00001];
ShapesController.t_counts = [100, 100, 100, 300, 300, 200, 200, 300, 500, 6000];
...
ShapesController.prototype.StartAnimation = function ()
{
    ...
    this.chelp = new CanvasHelp (chart_img.width, chart_img.height);
    
    this.canvas = {};
    this.canvas.chelp = new CanvasHelp (canvas_version.width, canvas_version.height);
    this.canvas.dx = canvas_version.width / 2;
    this.canvas.dy = canvas_version.height / 2;
    this.canvas.scale = Math.min (canvas_version.width / 2, canvas_version.height / 2);
    this.canvas.image = canvas_version;    

    this.shape_packer.DrawCanvas (this.canvas);
